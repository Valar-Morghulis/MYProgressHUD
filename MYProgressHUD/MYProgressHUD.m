...
- (void)setMode:(MYProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == MYProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == MYProgressHUDModeCustomView && self.customView != nil){
...
+ (MYProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
   return [MYProgressHUD showHUDAddedTo:view animated:animated canCancel:FALSE];//默认不可以
}

+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	UIView *viewToRemove = nil;
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[MYProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		MYProgressHUD *HUD = (MYProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"MYProgressHUDViewIsNillException" 
					format:@"The view used in the MYProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.mode = MYProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == MYProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == MYProgressHUDAnimationZoom) {
